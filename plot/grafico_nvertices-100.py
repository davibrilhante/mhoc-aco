...
if __name__ == "__main__":
    arq = open("resultados_totais-250.txt")
    arq.readline()
    budgets = [0.1, 0.2, 0.3]
    metr = int(sys.argv[1])
    ...
    '''
    plt.xticks([x-0.5 for x in range(len(array_y))],instances,size='small', rotation=45)
    plt.ylim([min(array_y+comp_y),max(array_y+comp_y)+1000])
    ...
    for i in range(len(array_y)):
        ...
        if max(array_y)<3: 
            ajuste = 0.01 
            arredonda = 3
        plt.text(i-0.3, array_y[i]+max(erro)+ajuste, str(round(array_y[i],arredonda)), size=12,rotation=45)
        '''
        if comp_y[i] <> 0:
    ...
    fig.savefig("e100"+metricas[metr]+nv[n_vertices]+"-"+str(budgets[custo])+'.png')
